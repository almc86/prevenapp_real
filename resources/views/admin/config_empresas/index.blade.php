  ...
  <div class="bg-white shadow-soft rounded-xl overflow-hidden">
    ...
    @if($empresas->count() > 0)
      <div class="border-t border-gray-200 px-6 py-4 bg-gray-50">
        <h4 class="text-sm font-medium text-gray-900 mb-3">Acceso rápido</h4>
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          @foreach($empresas->take(6) as $empresa)
            <a href="{{ route('admin.config-empresas.configs.index', $empresa) }}"
               class="group flex items-center p-3 bg-white rounded-lg border border-gray-200 hover:border-primary-300 hover:shadow-sm transition-all">
              <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-primary-100 group-hover:bg-primary-200 transition-colors">
                <i class="bx bx-building text-primary-600"></i>
              </div>
