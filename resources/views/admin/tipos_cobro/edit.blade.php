...
@section('title','Editar Tipo de Cobro')

@section('content')
<div class="container">
  <h3>Editar Tipo de Cobro</h3>
  ...
  <form method="POST" action="{{ route('admin.tipos-cobro.update', $tiposCobro) }}" id="tipoCobroForm">
    @csrf
    @method('PUT')
          ...
          <select name="empresa_principal_id" class="form-select" required>
            ...
            @foreach($empresas as $empresa)
              <option value="{{ $empresa->id }}"
                      {{ (old('empresa_principal_id', $tiposCobro->empresa_principal_id) == $empresa->id) ? 'selected' : '' }}>
      ...
      <div class="col-md-6">
        <div class="mb-3">
          ...
          <select name="empresa_contratista_id" class="form-select" required>
            ...
            @foreach($empresas as $empresa)
              <option value="{{ $empresa->id }}"
                      {{ (old('empresa_contratista_id', $tiposCobro->empresa_contratista_id) == $empresa->id) ? 'selected' : '' }}>
                {{ $empresa->nombre_empresa }} - {{ $empresa->rut_empresa }}
              </option>
            @endforeach
          </select>
      ...
      <div class="col-md-4">
        <div class="mb-3">
          ...
          <select name="tipo_cobro" class="form-select" required>
            ...
            <option value="pesos" {{ old('tipo_cobro', $tiposCobro->tipo_cobro) === 'pesos' ? 'selected' : '' }}>Pesos (CLP)</option>
            <option value="uf" {{ old('tipo_cobro', $tiposCobro->tipo_cobro) === 'uf' ? 'selected' : '' }}>UF</option>
          </select>
        </div>
      </div>
      <div class="col-md-4">
        <div class="mb-3">
          ...
          <select name="tipo_pago" class="form-select" required>
            ...
            <option value="factura" {{ old('tipo_pago', $tiposCobro->tipo_pago) === 'factura' ? 'selected' : '' }}>Factura</option>
            <option value="webpay" {{ old('tipo_pago', $tiposCobro->tipo_pago) === 'webpay' ? 'selected' : '' }}>WebPay</option>
          </select>
        </div>
      </div>
      <div class="col-md-4">
        <div class="mb-3">
          ...
          <select name="activo" class="form-select" required>
            <option value="1" {{ old('activo', $tiposCobro->activo) ? 'selected' : '' }}>Activo</option>
            <option value="0" {{ !old('activo', $tiposCobro->activo) ? 'selected' : '' }}>Inactivo</option>
          </select>
        </div>
      </div>
    </div>
    ...
    <div class="mb-3">
      ...
      <textarea name="observaciones" class="form-control" rows="3" maxlength="500"
                placeholder="Observaciones adicionales (opcional)">{{ old('observaciones', $tiposCobro->observaciones) }}</textarea>
    </div>
    ...
    <div class="mb-4">
      ...
      <div id="rangos-container">
        @php
          $rangosData = old('rangos', $tiposCobro->rangos->toArray());
        @endphp

        @if($rangosData)
          @foreach($rangosData as $index => $rango)
    ...
    <div class="d-flex gap-2">
      ...
      <a href="{{ route('admin.tipos-cobro.show', $tiposCobro) }}" class="btn btn-outline-secondary">Ver</a>
...
<script>
let rangoIndex = {{ count($rangosData ?? []) }};
