...
<div class="container">
  ...
  <form method="POST" action="{{ route('admin.usuarios.store') }}" enctype="multipart/form-data" id="form-user">
    @csrf
    ...
    <div class="row g-3">
      ...
      <div class="col-md-6 d-none" id="box_principales">
        ...
        <div class="form-text">Seleccione una o varias.</div>
      </div>

      {{-- Sección: Empresas Contratistas --}}
      ...
      {{-- Sección: Empresas Subcontratistas --}}
      ...
      {{-- Sección Prevencionista: SEREMI + Firma --}}
      <div class="col-md-6 d-none" id="box_prevencionista">
        <label class="form-label">N° Registro SEREMI</label>
        <input type="text" name="seremi_registro" class="form-control" value="{{ old('seremi_registro') }}">
        <div class="mt-2">
...
(function(){
  ...
  function toggleByRole() {
    ...
    // Reglas:
    // principal -> solo principales (requerido)
    // visualizador -> solo principales (requerido)
    // contratista/sub contratista/prevencionista -> principales + contratistas + subcontratistas
    // prevencionista -> además box_prevencionista
    if (selected === 'principal' || selected === 'visualizador') {
      ...
      // otros roles: no muestran cajas (ajusta si agregas más reglas)
    }
  }

  roleSelect.addEventListener('change', toggleByRole);
  toggleByRole(); // estado inicial (p/ old())
